...
function jupiterx_breadcrumb() {
	...
	// Open breadcrumb.
	jupiterx_open_markup_e( 'jupiterx_breadcrumb', 'ol', array( 'class' => 'breadcrumb' ) );
		$i = 0;
	...
	foreach ( $breadcrumbs as $breadcrumb_url => $breadcrumb ) {
		...
		if ( count( $breadcrumbs ) - 1 !== $i ) {
			jupiterx_open_markup_e( 'jupiterx_breadcrumb_item', 'li', [ 'class' => 'breadcrumb-item' ] );

				jupiterx_open_markup_e( 'jupiterx_breadcrumb_item_link', 'a', array( 'href' => $breadcrumb_url ) ); // Automatically escaped.

					// Used for mobile devices.
					jupiterx_open_markup_e( 'jupiterx_breadcrumb_item_link_inner', 'span' );
