...
(function ($) {
  ...
  var TemplateInstall = function () {
    ...
    _createClass(TemplateInstall, [{
      ...
      value: function getActions(partial) {
        if (partial) {
          return ['preparation', 'upload', 'unzip', 'validate', 'install_plugins', 'activate_plugins', 'theme_content', 'plugins_content', 'finalize'];
        }

        return ['preparation', 'backup_db', 'backup_media_records', 'reset_db', 'upload', 'unzip', 'validate', 'install_plugins', 'activate_plugins', 'theme_content', 'setup_pages', 'plugins_content', 'settings', 'menu_locations', 'theme_widget', 'restore_media_records', 'finalize'];
      }
    }, {
      key: 'run',
      value: function run() {
        ...
        $.ajax({
          ...
          success: function success(res) {
            ...
            if ('install_plugins' === actions[self.current] && res.install) {
              ...
              return;
            } else if ('activate_plugins' === actions[self.current] && res.activate) {
              $.ajax({
                type: 'POST',
                url: res['url'],
                data: res['activate'],
  ...
  var TemplatesSearch = function () {
    function TemplatesSearch(_ref2) {
      var node = _ref2.node,
          customImport = _ref2.customImport;
