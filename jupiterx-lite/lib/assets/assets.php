...
function jupiterx_enqueue_jupiterx_components() {
	...
	$plugins   = jupiterx_get_plugins_assets();
	...
	// ... .
	$scripts = array_merge( $theme['scripts'], $bootstrap['scripts'], $wc['scripts'], $plugins['scripts'] );
 ...
 */
function jupiterx_get_plugins_assets() {
	$assets = [
		'scripts' => [],
	];

	if ( class_exists( 'LazyLoad_Images' ) ) {
		$assets['scripts'][] = 'plugins/lazy-load';
	}
